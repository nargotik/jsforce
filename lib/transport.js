...
if (request.defaults) {
  ...
  if (parseInt(process.env.HTTP_MAXATTEMPTS)) {
    defaults.maxAttempts = parseInt(process.env.HTTP_MAXATTEMPTS);
  }
  if (parseInt(process.env.HTTP_RETRYDELAY)) {
    defaults.retryDelay = parseInt(process.env.HTTP_RETRYDELAY);
  }
